...
public interface LectureRepository extends JpaRepository<Lecture, Long> {
    ...
    // Soft delete를 고려한 조회
    @Query("SELECT l FROM Lecture l WHERE l.id = :id AND l.status IS TRUE")
    ...
    @Query("SELECT l FROM Lecture l " +
            ...
            "WHERE l.id = :id AND l.status IS TRUE")
    ...
    @Query("SELECT DISTINCT c " +
            ...
            "JOIN l.section s " +
            "JOIN s.course c " +
            "WHERE l.id = :id AND l.status = TRUE")
    Optional<Course> findCourseByLectureId(@Param("id") Long id);

    // 삭제되지 않은 모든 렉처 조회
    @Query("SELECT l FROM Lecture l WHERE l.status IS TRUE")
    List<Lecture> findAllNotDeleted();

    // 특정 섹션의 렉처 조회 (order 정렬, 삭제되지 않은 것만)
    @Query("SELECT l FROM Lecture l WHERE l.section.id = :sectionId AND l.status IS TRUE ORDER BY l.sortOrder ASC")
    List<Lecture> findBySectionIdAsc(Long sectionId);

    // 특정 섹션의 렉처 개수 조회 (삭제되지 않은 것만)
    @Query("SELECT COUNT(l) FROM Lecture l WHERE l.section.id = :sectionId AND l.status IS TRUE")
    Long countBySectionId(Long sectionId);

    // 특정 섹션의 최대 order 값 조회
    @Query("SELECT MAX(l.sortOrder) FROM Lecture l WHERE l.section.id = :sectionId AND l.status IS TRUE")
    Optional<Integer> findMaxOrderBySectionId(@Param("sectionId") Long sectionId);

    // 미리보기 가능한 렉처 조회 (특정 섹션)
    @Query("SELECT l FROM Lecture l WHERE l.section.id = :sectionId AND l.previewable IS TRUE AND l.status IS TRUE ORDER BY l.sortOrder ASC")
    List<Lecture> findBySectionIdPreviewableTrueAsc(Long sectionId);

    // 타입별 렉처 조회 (특정 섹션)
    @Query("SELECT l FROM Lecture l WHERE l.section.id = :sectionId AND l.type = :type AND l.status IS TRUE ORDER BY l.sortOrder ASC")
    List<Lecture> findBySectionIdAndTypeAsc(Long sectionId, LectureType type);

    // 특정 코스의 모든 렉처 조회 (섹션을 통해 조인)
    @Query("SELECT l FROM Lecture l JOIN l.section s WHERE s.course.id = :courseId AND l.status IS TRUE AND s.deletedAt IS NULL ORDER BY s.order ASC, l.sortOrder ASC")
